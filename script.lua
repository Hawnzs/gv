--[[
    GV Script - Refined Version
    - Fixed teleport execution
    - Better error handling
    - Robust character loading
...
--[[
    TELEPORT HANDLER
    This queues the script to re-execute after teleporting
]]
game.Players.LocalPlayer.OnTeleport:Connect(function(State)
    if State == Enum.TeleportState.InProgress then
        if _G.KeepInfYield and not _G.TeleportState.TeleportCheck and queueteleport then
            ...
            -- Payload to execute on the new server
            local payload = string.format([[
                ...
                if not success then
                    print("Failed to reload script after teleport: " .. tostring(err))
...
local v_Input = game:GetService("VirtualUser") -- For anti-AFK

--[[
    ANTI-AFK
    Prevents 20-minute idle disconnect
]]
local function antiAFK()
    if v_Input then
        ...
        -- Fallback anti-AFK using mouse movement
        task.spawn(function()
            local mouse = game:GetService("UserInputService")
            while task.wait(60) do
                pcall(function()
                    mouse:MouseMove(Vector2.new(0, 0))
                    task.wait(0.1)
                    mouse:MouseMove(Vector2.new(100, 100))
                end)
            end
        end)
    end
...
local function main()
    ...
    --[[
        CHARACTER LOADING
        Wait for character with timeout and retry
    ]]
    local function waitForCharacter()
        ...
        repeat
            ...
            -- Check if character exists and has HumanoidRootPart
            ...
            -- If character is dead or missing, wait for respawn
        ...
        until false -- Will break when character is valid
    end
    ...
    --[[
        REMOTE TRIGGER
        Try to spawn character if needed
    ]]
    ...
    -- Re-check character after camera setup
    ...
    local n_Gxb = 120      -- Grid spacing
    local n_Vcs = 0.06     -- Movement speed
    local n_Iuw = 40       -- Spiral radius
    local n_Ptl = 5        -- Time per player
    local n_Zmq2 = 50      -- Search height
    local n_Tmo = 60       -- Timeout before server hop
    ...
    local function f_Sho()
        ...
        -- Teleport to random available server
        if #t_Srv > 0 then
            _G.TeleportState.TeleportRetries = 0
    ...
    -- 10-minute fallback (prevents idle disconnect)
    task.spawn(function()
        task.wait(600)
        if not _G.TeleportState.ServerHopTimer and not _G.TeleportState.ScriptFinished then
    ...
    local function f_Ryn()
        ...
        if not hrp then
            ...
            repeat 
                ...
                attempts = attempts + 1
                if attempts > 20 then
                    print("Timeout waiting for HumanoidRootPart, exiting...")
                    return
                end
            until hrp
        end
        ...
        if not startHrp then 
            print("Failed to get HumanoidRootPart, exiting...")
            return 
        end
    ...
    --[[
        EXECUTE MAIN LOOP WITH ERROR HANDLING
    ]]
    local success, err = pcall(function()
        f_Ryn()
    end)
